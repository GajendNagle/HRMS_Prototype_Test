...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentFooter" runat="Server">
    ...
    <%--<script>
        ...
        var c = new Choices("#EventCategory", {
            searchEnabled: false
        });
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: [
                    { id: 1, title: "World Braille Day", start: "2024-06-04", className: "bg-info-subtle", allDay: true },
                    { id: 2, title: "World Leprosy Day", start: "2024-06-30", className: "bg-info-subtle", allDay: true },
                ],
                eventClick: function (info) {
                    ...
                    document.getElementById("event-location-tag").innerHTML = event.extendedProps.location || "No Location";
                    document.getElementById("event-description-tag").innerHTML = event.extendedProps.description || "No Description";
                    document.getElementById("event-title").value = event.title;
                    document.getElementById("event-location").value = event.extendedProps.location || "No Location";
                    document.getElementById("event-description").value = event.extendedProps.description || "No Description";
                    document.getElementById("eventid").value = event.id;

                    if (event.classNames[0]) {
                        c.destroy();
                        c = new Choices("#EventCategory", { searchEnabled: false });
                        c.setChoiceByValue(event.classNames[0]);
                    }
                    ...
                    let end = event.end;
                    let endAdjusted = end ? new Date(end).setDate(new Date(end).getDate() - 1) : null;
                    let dateRange = end ? `${formatDate(start)} to ${formatDate(endAdjusted)}` : formatDate(start);
                    ...
                    document.getElementById("event-start-date-tag").innerHTML = dateRange;
                    let startTime = getTime(event.start);
                    let endTime = getTime(event.end);
                ...
                dateClick: function (info) {
                    // openModal(false); Uncomment and define openModal if needed.
                },
                ...
                dayRender: function (info) {
                    const dayHasEvents = calendar.getEvents().some(event => {
                        return event.start.toDateString() === info.date.toDateString();
                    });
                    if (!dayHasEvents) {
                        info.el.querySelector('.fc-daygrid-day-events').style.display = 'none';
                    }
                }
            });
            ...
            function eventClicked() {
                ...
                document.getElementById("EventCategory").classList.replace("d-block", "d-none");
